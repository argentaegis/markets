"""BacktestConfig — run parameters: symbol, dates, timeframe, data, fees, fills.
...
initial_cash, fee_config, fill_config drive the engine loop (A4: configuration-first).
"""
...
def _fee_config_to_dict(cfg: FeeModelConfig | None) -> dict | None:
    """Serialize FeeModelConfig for JSON, or None."""
    if cfg is None:
        return None
    return {"per_contract": cfg.per_contract, "per_order": cfg.per_order}
...
def _fee_config_from_dict(d: dict | None) -> FeeModelConfig | None:
    """Restore FeeModelConfig from dict, or None.

    Reasoning: Lazy import breaks circular dependency (domain -> broker -> domain).
    """
    if not d:
        return None
    from src.broker.fee_model import FeeModelConfig as _FeeModelConfig

    return _FeeModelConfig(
        per_contract=float(d.get("per_contract", 0.0)),
        per_order=float(d.get("per_order", 0.0)),
    )
...
class BacktestConfig:
    ...
    Reasoning: A4 configuration-first. initial_cash, fee_config, fill_config
    added for engine loop wiring. Defaults preserve backward compatibility.
    """
    ...
    def to_dict(self) -> dict:
        ...
        fee_dict = _fee_config_to_dict(self.fee_config)
    ...
    def from_dict(cls, d: dict) -> BacktestConfig:
        ...
        return cls(
            ...
            fee_config=_fee_config_from_dict(d.get("fee_config")),
